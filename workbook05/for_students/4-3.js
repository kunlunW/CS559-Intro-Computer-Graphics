...
function myTriAL(t) {
  let length1 = Math.sqrt(120*120 + 50*50);
  let length2 = 120;
  let length3 = 50;
  let st = t * (length1 + length2 + length3)/ 3;  
  if (st<length1){
    return(lerpPt(triPt1,triPt2,st/length1));  
  } 
  else if (st<length1 + length2) {
    return(lerpPt(triPt2,triPt3,(st-length1)/length2));
  }
  else  {
    return lerpPt(triPt3,triPt1,(st-length1-length2)/length3);
  }           
}
