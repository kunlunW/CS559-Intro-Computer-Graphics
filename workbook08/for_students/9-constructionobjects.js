...
export class GrTruck extends GrObject {
  ...
  constructor(params={})  {
    ...
    tire[0].position.set(0.4,0.55,2.4);
		...
		tire[2].position.set(3,0.55,2.4);
		tire[3].position.set(3,0.55,-0.4);
		tire[4].position.set(4.5,0.55,2.4);
    tire[5].position.set(4.5,0.55,-0.4);
    tire[6].position.set(7, 0.55, 2.4);
    tire[7].position.set(7, 0.55, -0.4);
    
    for(let i=0; i<8; i++) {
      Truck.add(tire[i]);
    }
    // drawing the bottom support of the truck 
    let supportGeometry = new T.BoxGeometry(6,0.5,2.5);
    let supportMaterial = new T.MeshStandardMaterial({color: "gray"});
    let support = new T.Mesh(supportGeometry, supportMaterial);
    support.position.set(4.55,0.6,1);
    Truck.add(support);
    ...
    let truckHead = new T.Shape();
		...
		truckHead.lineTo(-2, 1);
    truckHead.lineTo(0, 0);
    let headGeometry = new T.ExtrudeGeometry(truckHead, exSettings);
		let headMaterial = new T.MeshStandardMaterial({color: "blue"});
		let head = new T.Mesh(headGeometry, headMaterial);
    head.translateY(0.5);
    Truck.add(head);
    // drawing the container of the truck 
    let container = new T.Group();
    let containerGeometry = new T.BoxGeometry(5.5,0.3,2.5);
		let containerMaterial = new T.MeshStandardMaterial({color: "yellow"});
    let botSupport = new T.Mesh(containerGeometry, containerMaterial );
    botSupport.position.set(1, 0.1, 0);
    
    // drawing the walls of the container
    let wallLeftGeometry = new T.BoxGeometry(5.5,2.5,0.2);
    let wallRightGeometry = new T.BoxGeometry(5.5,2.5,0.2); 
		let wallLeft = new T.Mesh(wallLeftGeometry, containerMaterial );
    let wallRight = new T.Mesh(wallRightGeometry, containerMaterial );
		wallLeft.position.set(1,0.75,0.9);
		wallRight.position.set(1,0.75,-0.9);
    
    // drawing the front wall of the container
    let wallFrontGeometry = new T.BoxGeometry(0.2,3,2);
		let wallFront = new T.Mesh(wallFrontGeometry, containerMaterial );
		wallFront.position.set(-1.65,1.05,0);
    
    // drawing the back wall of the container
    let wallBackGeometry = new T.BoxGeometry(0.2,2,2);
		let wallBack = new T.Mesh(wallBackGeometry, containerMaterial );
		wallBack.translateY(-0.5);
    
    // setting the back wall control so that when we are dumping 
    // things out of the truck we can move the back wall of the 
    // container 
		let backWallControl = new T.Group();
		backWallControl.add(wallBack);
		backWallControl.position.set(3.8,1.5,0);
    
    // adding things to the container 
		container.add(botSupport);
		container.add(wallLeft);
		container.add(wallRight);
		container.add(wallFront);
		container.add(backWallControl);
		container.translateX(-1);
    
    // setting the container control so that we can 
    // move the truck 
		let containerControl = new T.Group();
		containerControl.add(container);
		containerControl.position.set(5.75,0.8,1);
    Truck.add(containerControl);

    // setting up the control panel 
    super(`Truck-${dumpTruckObCtr++}`,Truck,
			  [ ['x',-8,8,-5],
				['z',-8,8,-4],
				['theta',0,360,0],
				['bucket',0,50,0]
		...
		this.whole_ob = Truck;
		this.bucket = containerControl;
		this.back = backWallControl;
}
update(paramValues){
  this.whole_ob.position.x = paramValues[0];
  this.whole_ob.position.z = paramValues[1];
  this.whole_ob.rotation.y = degreesToRadians(paramValues[2]);
  this.bucket.rotation.z = -1*degreesToRadians(paramValues[3]);
  this.back.rotation.z = degreesToRadians(paramValues[3]);
